#!/usr/bin/env python
# 
# This script implements the Neon Neighbour Search caching strategy.
#
...
class nbsearch_neon(object):
    def __init__(self, router, cachesize):
        self.router = router
        self.cache = cache_lru(cachesize)
        self.logfh = None
        self.nbbf = {}                   # A dict stores all the neighbours' bloomfiltes
        self.error_rate = 0.001          # The error rate of bloom filter
        self.vcover = self.vertex_cover(self.router.topology.keys())
    ...
    def update2neighbour(self):
        ...
        for nb in self.router.neighbours(self.router.vrid):
            ...
            hdr.dst = nb
            data = self.get_cache_bloomfilter(nb)
            hdr.data = data.bitarray.tostring()
            self.router.send(hdr)
        pass

    def get_cache_bloomfilter(self, nbid):
        bf = self.create_empty_bloomfilter()
        for cid in self.cache.keys:
            bf.add(cid)
        if (self.router.vrid in self.vcover and
            nbid not in self.vcover) :
            for tid, tbf in self.nbbf.items():
                if tid != nbid:
                    bf |= tbf
        return bf
    ...
    def vertex_cover(self, edges):
        """Calculate vertex cover given a graph."""
        C = set()
        E = edges
        while len(E):
            x, y = E.pop()
