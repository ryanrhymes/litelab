#!/usr/bin/env python
# 
# This script implements the Green INCA Neighbour Search caching strategy.
# The difference between this one and traditional nbsc is it takes neighbors'
# green energy into account.
#
...
class green_nbscg(object):
    def __init__(self, router, cachesize):
        router.greenmap = router.manager.dict()
        self.router = router
        self.router.build_routing_table(self.router.vrid)
    ...
    def query_neighbour(self, hdr):
        found = False
        nbs = []
        tmst = int(time.time()) / TIME_SCALE
        if hdr.ttl > 0:
            for nb, bf in self.nbbf.items():
                if hdr.id in bf:
                    gr = self.router.greenmap[nb].get(tmst, 0)
                    nbs.append((gr, nb))
        if len(nbs) > 0:
            found = True
            nbs.sort(reverse=1)
